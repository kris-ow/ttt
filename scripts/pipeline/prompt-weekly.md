...
- **Do not propagate arithmetic that does not check out.** If a daily brief states a derived figure alongside the inputs it came from (a contract total and a unit count, a total spend and a per-unit price) and the two do not agree, use the figure the stated inputs support, or drop the derived one — never repeat it as fact. Where a brief already flags the inconsistency, keep that flag rather than silently resolving it.
- **Never carry a Cybertruck/Cybercab hedge forward.** Auto-captions garble "Cybercab" as "Cybertruck", so a daily brief occasionally names both ("85 Cybertrucks/Cybercabs", "Cybercab/Cybertruck design"). Resolve it from the surrounding context — robotaxi deployment, ride-hailing fleets and the Cybercab ramp mean Cybercab; pickup sales, tooling and recalls mean Cybertruck — and write the single correct name. Never reproduce the slashed pair.
