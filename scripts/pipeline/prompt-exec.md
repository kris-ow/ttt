...
- Never describe SpaceX as a private company, and never label a current SPCX price, valuation, or market move "private", "private-market", or "privately valued". If a source summary carries that stale framing, drop the "private" wording and write the fact plainly — do not propagate it.
...
- **Do not propagate arithmetic that does not check out.** If a source summary states a derived figure alongside the inputs it came from (a contract total and a unit count, a total spend and a per-unit price) and the two do not agree, use the figure the stated inputs support, or drop the derived one — never repeat it as fact. Where a source summary already flags the inconsistency, keep that flag rather than silently resolving it.
- **Never carry a Cybertruck/Cybercab hedge forward.** Auto-captions garble "Cybercab" as "Cybertruck", so a source summary occasionally names both ("85 Cybertrucks/Cybercabs", "Cybercab/Cybertruck design"). Resolve it from the surrounding context — robotaxi deployment, ride-hailing fleets and the Cybercab ramp mean Cybercab; pickup sales, tooling and recalls mean Cybertruck — and write the single correct name. Never reproduce the slashed pair.
