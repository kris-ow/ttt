...
- `weekly-summary.js` — Weekly Brief aggregating the prior 7 daily briefs (Mon-Sun) + last 4 weekly briefs (continuation context) into `news/YYYYMMDD_ttt_99_weekly_brief_summary.txt`. Direct API (Sonnet 4.6). Runs on a dedicated Monday-only cron `45 5 * * 1` (off the :00/:15 stampede so it actually fires near schedule). Robotaxi tracker table uses Now/7D/30D columns (no 1D).
