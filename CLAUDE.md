...
- `exec-summary.js` — Daily Tesla Brief aggregating all `_summary.txt` for a date into `news/YYYYMMDD_ttt_00_executive_summary.txt` (filename kept; user-facing Title is "Daily Tesla Brief"). Direct API (Sonnet 5, thinking disabled). Runs only on the 00:15 and 07:15 UTC pipeline executions.
...
Uses Claude Sonnet 5 via Batch API (50% cheaper); switched from Sonnet 4.6 on 2026-07-03 after an A/B test (better attribution/hedging + relevance-gate judgment). All pipeline calls pass `thinking: THINKING` (disabled, from `config.js`) — Sonnet 5 silently runs adaptive thinking when the param is omitted, which would bill thinking tokens and risk truncation. `max_tokens` 16000 (Sonnet 5's tokenizer yields ~30% more tokens for the same text). **Pricing constants in `config.js` are Sonnet 5 INTRO rates — flip to sticker $3/$15 on 2026-09-01** and reassess whether quality justifies the ~+40% cost vs 4.6. Prompt mitigations for Sonnet 5 quirks: unresolvable-garble fallback rule (refer generically, never quote garbled forms) and output-tightness guidance. **Attribution/quote-integrity rules added to all four prompts 2026-07-13** after an Electrek editorial assertion escalated through summary→daily→weekly into a fabricated Tesla "admission" (quote marks added around a paraphrase, attribution stripped): summarizers must keep unsourced "company admitted X" claims attributed to the outlet/host, aggregators must never strip attribution (keep it or drop the claim), and quotation marks are only for verbatim source text. Summaries match existing `_summary.txt` format.
