...
- `weekly-summary.js` — Weekly Brief aggregating the prior 7 daily briefs (Mon-Sun) + last 4 weekly briefs (continuation context) into `news/YYYYMMDD_ttt_99_weekly_brief_summary.txt`. Direct API (Sonnet 5, thinking disabled). Primary trigger since 2026-07-06: the Mac Mini dispatches `daily-pipeline.yml` (`workflow_dispatch`, `mode=direct`, `skip_exec=true`) at 05:45 UTC Mondays — GitHub's scheduler delays cron runs ~4–4.5h regardless of slot (the old cron landed ~10:00 UTC every week), while dispatch runs start in seconds. The Monday-only cron `45 5 * * 1` is kept as a delayed fallback; the script skips if the week's brief file already exists (`--force` to regenerate). Output is post-processed by `normalizeSeparators()` to guarantee exactly one `---` between `## ` sections — the model emits them inconsistently and `reddit-weekly.js` keys on them.
- `reddit-weekly.js` — deterministic formatter (no LLM) that converts the latest Weekly Brief into a copy-paste Reddit post file (`data/reddit-weekly/YYYY-MM-DD.md`, target r/teslainvestorsclub) with TWO variants: **Variant A** (link-embed experiment, added 2026-07-02) = the `/w/<date>/` deep-link URL (paste + Reddit "Link Embed" → OG card) followed by the Brief's headline bullets verbatim, topped up with category first-bullets ("A few of the details:") ONLY until the body clears 1000 chars (excl. the URL), then a closer counting the held-back items — r/teslainvestorsclub requires ≥1000 chars of body text once any body text is present (rule discovered on the 2026-07-06 post; the short Brief-bullets list alone is ~500 chars), and the post should stay a quick funnel to the link, not a second copy of the brief (~2000 chars rejected as too long). The file header prints the Variant A char count and the script warns loudly if under; **Variant B** (fallback) = the original full-text post with the TTT link in the Brief heading (top-of-fold, single outbound link). Runs right after the weekly brief on the Monday cron. If a transform's pattern doesn't match (source format drift), it warns loudly: `!! WARNING` lines in the output file header + `::warning::` GitHub Actions annotation. If the week's OG card (`public/og/weekly-<date>.png`) isn't committed yet — always true on the Monday CI run, since cards are rendered locally — it adds a `>> TODO` reminder line to the header (run `npm run og-images`, commit + push, wait for deploy before posting Variant A).
