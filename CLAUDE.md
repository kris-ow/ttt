...
- `weekly-summary.js` — Weekly Brief aggregating the prior 7 daily briefs (Mon-Sun) + last 4 weekly briefs (continuation context) into `news/YYYYMMDD_ttt_99_weekly_brief_summary.txt`. Direct API (Sonnet 5, thinking disabled). Runs on a dedicated Monday-only cron `45 5 * * 1` (off the :00/:15 stampede so it actually fires near schedule). Output is post-processed by `normalizeSeparators()` to guarantee exactly one `---` between `## ` sections — the model emits them inconsistently and `reddit-weekly.js` keys on them.
