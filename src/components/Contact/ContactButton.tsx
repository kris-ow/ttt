...
export function ContactButton({ onClick, className = '' }: { onClick: () => void; className?: string }) {
  return (
    <button
      ...
      className={`h-[30px] w-[30px] shrink-0 items-center justify-center border cursor-pointer transition-colors border-green/50 text-green/50 hover:border-green hover:text-green ${className}`}
    >
